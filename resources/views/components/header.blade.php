<header class="bg-blue-900 text-white p-4" x-data="{ open: false }">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            ...
            <x-nav-link url="/" :active="request()->is('/')" icon="house">Home</x-nav-link>
            <x-nav-link url="/jobs" :active="request()->is('jobs')" icon="star">All Jobs</x-nav-link>
            @auth
                <x-nav-link url="/jobs/saved" :active="request()->is('jobs/saved')" icon="bell">Saved Jobs</x-nav-link>
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard') }}">
                        ...
                        @else
                            <img src="{{ asset('storage/avatars/default.jpg') }}" alt="{{ Auth::user()->name }}"
                                 class="w-10 h-10 rounded-full">
                        @endif
                    </a>
                </div>
            @else
                ...
                <x-nav-link url="/register" :active="request()->is('register')" icon="circle-user">Register</x-nav-link>
            @endauth
        </nav>
